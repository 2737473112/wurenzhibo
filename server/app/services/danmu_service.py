from ..models import User, SpeechLibrary,QALibrary
import random
...
class DanmuService:
    ...
    def synthesize_audio(self, text):
       # print("开始合成音频")
        ...
        predict_data = {
            ...
            #"speaking_length": float(self.speech_speed)   # 使用传入的速度参数
            "speaking_length": 1
        }
    ...
    def start_danmu_response(self):
        ...
        while self.running:
            try:
                ...
                # 把所有弹幕移动到临时列表
                while self.danmu_queue:
                    temp_danmu_list.append(self.danmu_queue.pop(0))

                # 现在处理临时列表中的每个弹幕
                for danmu_data in temp_danmu_list:
                    try:
                        ...
                        if reply:
                            print("回复弹幕:", reply)
                            # 生成音频并添加到音频队列
                            # print(self.speech_speed)
                            # print(self.voice)
                            audio_data = self.synthesize_audio(reply)
                ...
                print(f"Error in danmu response loop: {e}")
                # 可以在这里添加额外的代码，比如重置状态或资源


    def generate_danmu_audio(self, danmu_data):
        try:
            reply = self.respond_to_danmu(danmu_data)
            if reply:
                print("回复弹幕:", reply)
                # 生成音频并返回音频路径
                audio_path = self.synthesize_audio(reply)
                return audio_path
            else:
                #print("没有弹幕回复")
                return None
    ...
    def get_all_audio(self):
        ...
        while not self.audio_queue.empty():
            ...
            audio_list.append({"audio_path": audio_path, "type": "danmu_audio"})
         
        return audio_list

    def stop_danmu_response(self):
        self.running = False